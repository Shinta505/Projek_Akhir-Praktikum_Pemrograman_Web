...
if (empty($_SESSION['pasienID'])) {
    header("location:login.php?pesan=gamasuk");
}
?>
...
<body style="background-color: #FEF9EF; color: #213555;">
    ...
    <ul class="nav justify-content-between bg-transparent mt-3 mb-3" style="margin-left: 11rem; margin-right: 2rem;">
        <li class="nav-item">
            <h1 class="fs-1 fw-bold">Buat Jadwal Kunjungan</h1>
        </li>
        <li class="nav-item">
    ...
    <div class="row">
        <div class="col-8">
            <form class="mx-3" action="input_janji3.php" method="POST">
                <!-- //nama ortu, nama anak  -->
                ...
                $id = $_GET['dokterID'];
                $query = mysqli_query($connect, "select * from dokter where dokterID = '$id'");
                $data = mysqli_fetch_array($query);
                ?>
                <select name="dokter" id="dokter" class="py-1 ms-4 bg-transparent" size="1" style="width: 39%; background-color: #A2D2FF;">
                    <option value="<?php echo $data['dokterID'] ?>" selected><?php echo $data['nama'] . " - " . $data['lokasi'] ?></option>
                </select>
                <br>
